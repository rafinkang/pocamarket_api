...
public class GoogleOAuth2Service implements OAuth2UserService {
    ...
    public String getAccessToken(String code, String redirectUri) {
        ...
        // JWT 토큰인지 확인 (3개의 점으로 구분된 구조)
        if (isJwtToken(code)) {
            log.info("JWT 토큰(ID Token) 감지: access token 요청을 건너뛰고 JWT 토큰을 반환");
            return code; // JWT 토큰을 그대로 반환
        }
    ...
    public OAuth2UserInfoDto getUserInfo(String accessToken) {
        ...
        if (isJwtToken(accessToken)) {
            log.info("JWT 토큰(ID Token)에서 사용자 정보 추출");
            return getUserInfoFromJwtToken(accessToken);
        }
        ...
        try {
            ...
            log.info("Google OAuth2 사용자 정보 획득 성공: providerId={}, email={}", userInfo.getProviderId(),
                    userInfo.getEmail());
            return userInfo;
     ...
     * @return 사용자 정보 DTO
     */
    private OAuth2UserInfoDto getUserInfoFromJwtToken(String jwtToken) {
        try {
            ...
            log.info("JWT 토큰 payload: {}", payload);
            ...
            OAuth2UserInfoDto userInfo = OAuth2UserInfoDto.builder()
                    .provider(PROVIDER_TYPE)
                    .providerId(jsonNode.get("sub").asText()) // subject ID
                    .email(jsonNode.has("email") ? jsonNode.get("email").asText() : null)
                    .name(jsonNode.has("name") ? jsonNode.get("name").asText() : null)
                    .nickname(jsonNode.has("given_name") ? jsonNode.get("given_name").asText() : null)
                    .profileImageUrl(jsonNode.has("picture") ? jsonNode.get("picture").asText() : null)
            ...
            log.info("JWT 토큰에서 사용자 정보 추출 성공: providerId={}, email={}", userInfo.getProviderId(), userInfo.getEmail());
            return userInfo;

        } catch (Exception e) {
            log.error("JWT 토큰 파싱 실패", e);
            throw new UserException(UserErrorCode.UNKNOWN_ERROR, "JWT 토큰 파싱에 실패했습니다.");
        }
    }
